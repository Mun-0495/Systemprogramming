...
void eval(char *cmdline)
{
  ...
  if(!builtin_cmd(argv)){ 
    ...
    /* Block SIGINT ans save previous blocked set*/
    ...
    if(pid < 0) {
		...
		if(pid == 0) {
      /* UnBlock SIGINT ans save previous blocked set*/
      //setpgid(0, 0);
      if(setpgid(0, 0) < 0) {
...
int builtin_cmd(char **argv)
{
	if(!strcmp(argv[0], "quit")) //trace 02.txt
		exit(0);
...
void do_bgfg(char **argv)
{
  ...
  //pid idenfies jid
    ...
    // check if process id identifies a valid job
    if (job == NULL) 
    {
  ...
  // argument is neither a PID or JID
  else { 
  ...
  // put job to foreground
    ...
    // wait until foreground job is done
    waitfg(job->pid); 
  }
  return;
}
...
void sigchld_handler(int sig)
{
  ...
  while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) { //find all zombie
    // if child job terminated because it recieved a signals
    ...
    //signal cause by child
    else if(WIFSIGNALED(status)) {
      // print which signal caused termination, WTERMSIG can do it.
...
void sigint_handler(int sig)
{
  // current foreground jobs
  pid_t pid = fgpid(jobs); 
  if(pid) 
    // send signal 'sig' to entire foreground process group
    // kill(-pid, sig);
    if (kill(-pid, sig) < 0) 
